...
- 以前这里只告诉你“炸了”，但不知道是哪类城市参数把它炸掉。
- 现在 fatal 前会把冠层高度、粗糙度相关量、建筑统计量一起打出来，能直接判断是 `ZA` 太低，还是 `MH/HGT/LP/LB/LF` 本身异常。
...
- 这里我把它表述为“与源码修补一致的输出变化”，不把文件大小单独当成唯一证据；真正的核心证据仍然是上一节里备份 `wrfinput` 的变量内容变化。
